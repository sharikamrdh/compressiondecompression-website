...
def decompress():
    ...
    if os.path.exists(file_path):
        filename, ext = os.path.splitext(os.path.basename(request.form['file_path']))
        decompressed_path = os.path.join(app.config['DOWNLOAD_FOLDER'], filename)

        showProgressBar()
        
        if ext == '.compressed':
            decompress_lossless(file_path, decompressed_path)
        elif ext in ['.jpg', '.jpeg', '.png', '.gif', '.mp3', '.wav', '.ogg', '.mp4', '.avi', '.mov', '.txt', '.csv', '.doc', '.docx', '.xls', '.xlsx']:
            decompress_lossy(file_path, decompressed_path)
        ...
        return render_template('decompression_result.html')
